...
def write_test_data():
    ...
    test_dir = '/d/bip3/ezbc/taurus/data/planck/test_data/'

    # Write out av data
    (data, header) = extract_test_data()
    (data, header) = tau353_to_ebv(data, header)
    (data, header) = ebv2av(data, header)
    write_data(data, header, test_dir + 'test_taurus_av_planck.fits')

    header_file = pf.Header(header)
    header = pf.Header(header)

    # Write the header
    header_file.tofile(test_dir + 'test_taurus_av_planck.header',
                   clobber=True)

    # Compare map with Jouni's K09
    av_image, av_header = pf.getdata(test_dir + \
            'taurus_av_kainulainen2009_nan.fits', header=True)

    av_hdu = pf.open(test_dir + 'taurus_av_kainulainen2009_nan.fits')
    av_data, av_header = av_hdu[0].data, av_hdu[0].header

    #
    os.chdir(test_dir)

    planck_data, planck_header = regrid_images('test_taurus_av_planck',
            'taurus_av_kainulainen2009_nan')
    ...
    plot_av_image(av_image=av_image,
            ...
            contours=[3,7,11],
            colorbar_lims=(-3,14),
            title='taurus: Kainulainen 09 map with Planck Contours',
            savedir=test_dir,
            filename='taurus_k09_planck_map.png',
            show=False)
